...
export class LedgerNodeTransport {
  public readonly type: string = 'node_ledger';
  public readonly byteLimit = 1024; // not sure what this should be yet
  ...
  private async exchange(dongle: TransportNodeHid, message: Buffer): Promise<LedgerResponse> {
  ...
  private async getLedger(): Promise<TransportNodeHid> {
    ...
    if (paths.length > 0) {
      ...
      return TransportNodeHid.open(path);
    }
