import { Witness as WitnessCore } from '@neo-one/client-core';
...
export interface TransportNode {
  ...
  readonly sign: (
    options: {
      ...
      readonly key: BufferString; // sometimes we need public, sometimes private
    },
  ) => Promise<WitnessCore>;
}
...
export interface KeyStoreOptions {
  ...
  readonly transport: TransportNode;
}
...
export abstract class TransportStore {
  public readonly type: string;
  public readonly transport: TransportNode;
  ...
  public constructor(options: KeyStoreOptions) {
    ...
    if (options.store.getWalletsSync !== undefined) {
      this.initWithWallets(options.store.getWalletsSync());
    }
  ...
  public async sign({
    ...
    return this.capture(
      async () => {
        const messageBuff = Buffer.from(message, 'hex');
        const key = this.transport.type === 'local' ? this.getPrivateKey(account) : this.getPublicKey(account);
        const witness = await this.transport.sign({
          message: messageBuff,
          key,
        });
      ...
      `neo_${this.transport.type}_sign`,
      monitor,
    );
  }
  ...
  private getPublicKey(id: UserAccountID): BufferString {
