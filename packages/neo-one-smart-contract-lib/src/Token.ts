...
export abstract class Token<Decimals extends number> extends SmartContract {
  ...
  private readonly balances = MapStorage.for<Address, Fixed<Decimals>>();
  private readonly approvedTransfers = MapStorage.for<[Address, Address], Fixed<Decimals>>();
  private mutableSupply: Fixed<Decimals> = 0;

  private readonly notifyTransfer = createEventNotifier<Address | undefined, Address | undefined, Fixed<Decimals>>(
    'transfer',
    'from',
    'to',
    'amount',
  );
  private readonly notifyApproveSendTransfer = createEventNotifier<Address, Address, Fixed<Decimals>>(
    'approveSendTransfer',
    'from',
    'to',
    'amount',
  );
  private readonly notifyRevokeSendTransfer = createEventNotifier<Address, Address, Fixed<Decimals>>(
    'revokeSendTransfer',
    'from',
    'to',
    'amount',
  ...
  @constant
  public approvedTransfer(from: Address, to: Address): Fixed<Decimals> {
  ...
  public approveReceiveTransfer(_from: Address, _amount: Fixed<Decimals>, _asset: Address): boolean {
    return false;
  }

  public revokeSendTransfer(from: Address, to: Address, amount: Fixed<Decimals>): boolean {
    if (amount < 0) {
      throw new Error(`Amount must be greater than 0: ${amount}`);
    }
  ...
  public onRevokeSendTransfer(_from: Address, _amount: Fixed<Decimals>, _asset: Address): void {
    // do nothing
  }
  ...
  protected burn(addr: Address, amount: Fixed<Decimals>): boolean {
