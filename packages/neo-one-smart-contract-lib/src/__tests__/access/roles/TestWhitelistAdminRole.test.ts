...
describe('test whitelist_admin access role', () => {
  test('deploy, initialize, add, remove', async () => {
    await withContracts<{ testWhitelistAdmin: SmartContractAny }>(
      [
        {
          filePath: path.resolve(
            ...
            'access',
            'roles',
            'TestWhitelistAdmin.ts',
          ),
          name: 'TestWhitelistAdmin',
        },
      ],
      async ({ client, networkName, testWhitelistAdmin: smartContract, masterAccountID }) => {
        crypto.addPublicKey(
        ...
        const [ownerIsWhitelistAdminResult, alternateIsWhitelistAdminResult] = await Promise.all([
          smartContract.isWhitelistAdmin(masterAccountID.address),
          smartContract.isWhitelistAdmin(alternate.userAccount.id.address),
        ]);

        expect(ownerIsWhitelistAdminResult).toBeTruthy();
        expect(alternateIsWhitelistAdminResult).toBeFalsy();

        /* blatant lie */
        const invalidOpResult = await smartContract.addWhitelistAdmin(
        ...
        const invalidOpResult2 = await smartContract.removeWhitelistAdmin(
          masterAccountID.address,
          alternate.userAccount.id.address,
          { from: alternate.userAccount.id },
        );
        ...
        const validOpResult = await smartContract.addWhitelistAdmin(
          alternate.userAccount.id.address,
          masterAccountID.address,
          {
        ...
        expect(validOpReceipt.events[0].name).toEqual('add whitelist admin');

        const [ownerIsWhitelistAdminResultB, alternateIsWhitelistAdminResultB] = await Promise.all([
          smartContract.isWhitelistAdmin(masterAccountID.address, { from: masterAccountID }),
          smartContract.isWhitelistAdmin(alternate.userAccount.id.address, { from: alternate.userAccount.id }),
        ]);

        expect(ownerIsWhitelistAdminResultB).toBeTruthy();
        expect(alternateIsWhitelistAdminResultB).toBeTruthy();

        const validOpResult2 = await smartContract.removeWhitelistAdmin(
          masterAccountID.address,
          alternate.userAccount.id.address,
          { from: alternate.userAccount.id },
        );
        ...
        expect(validOpReceipt2.events[0].name).toEqual('remove whitelist admin');

        const [ownerIsWhitelistAdminResultC, alternateIsWhitelistAdminResultC] = await Promise.all([
          smartContract.isWhitelistAdmin(masterAccountID.address),
          smartContract.isWhitelistAdmin(alternate.userAccount.id.address),
        ]);

        expect(alternateIsWhitelistAdminResultC).toBeTruthy();
        expect(ownerIsWhitelistAdminResultC).toBeFalsy();
      },
      { deploy: false },
    );
  });
