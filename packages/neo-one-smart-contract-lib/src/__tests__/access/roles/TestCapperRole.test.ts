...
describe('test capper access role', () => {
  test('deploy, initialize, add, remove', async () => {
    await withContracts<{ testCapper: SmartContractAny }>(
      [
        {
      ...
      async ({ client, networkName, testCapper: smartContract, masterAccountID }) => {
        crypto.addPublicKey(
        ...
        expect(ownerIsCapperResult).toBeTruthy();
        expect(alternateIsCapperResult).toBeFalsy();

        /* blatant lie */
        const invalidOpResult = await smartContract.addCapper(
        ...
        const invalidOpResult2 = await smartContract.removeCapper(
          masterAccountID.address,
          alternate.userAccount.id.address,
          { from: alternate.userAccount.id },
        );
        ...
        const validOpResult = await smartContract.addCapper(alternate.userAccount.id.address, masterAccountID.address, {
        ...
        expect(validOpReceipt.events[0].name).toEqual('add capper');

        const [ownerIsCapperResultB, alternateIsCapperResultB] = await Promise.all([
          smartContract.isCapper(masterAccountID.address, { from: masterAccountID }),
          smartContract.isCapper(alternate.userAccount.id.address, { from: alternate.userAccount.id }),
        ]);

        expect(ownerIsCapperResultB).toBeTruthy();
        expect(alternateIsCapperResultB).toBeTruthy();

        const validOpResult2 = await smartContract.removeCapper(
          masterAccountID.address,
          alternate.userAccount.id.address,
          { from: alternate.userAccount.id },
        );
        ...
        expect(validOpReceipt2.events[0].name).toEqual('remove capper');
        ...
        expect(alternateIsCapperResultC).toBeTruthy();
        expect(ownerIsCapperResultC).toBeFalsy();
      },
      { deploy: false },
    );
  });
