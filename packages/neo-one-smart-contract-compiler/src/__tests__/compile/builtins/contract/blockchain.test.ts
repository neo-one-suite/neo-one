...
describe('Blockchain', () => {
  ...
  test('currentTransaction', async () => {
    ...
    const sender = 'NfdKztHauMe8n8HnSbzCUZCNfQY7gToy2q';
    const validUntilBlock = 2102400;
    const systemFee = 0;
    const networkFee = 0;
    ...
    await node.executeString(
      ...
      // assertEqual(transaction.notifications, []);
      assertEqual(transaction instanceof Transaction, true);
    `,
      {
        // TODO: UAPB.invokeRaw() currently doesn't add these to the transaction script. this currently does nothing
        transfers: [
          {
            to: keys[0].address,
