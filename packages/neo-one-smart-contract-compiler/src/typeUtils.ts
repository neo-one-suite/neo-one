...
export const hasType = (type: Type | undefined, isType: (type: Type) => boolean): boolean =>
  type !== undefined && (isType(type) || hasUnionType(type, isType) || hasIntersectionType(type, isType));
...
export const getTypes = (typeIn: Type | undefined, isType: (type: Type) => boolean): ReadonlyArray<Type> => {
  if (typeIn === undefined) {
    return [];
  }
  const type = getType(typeIn);
  ...
  if (isUnion(type)) {
    return type
      .getUnionTypes()
      .reduce<ReadonlyArray<Type>>((acc, unionType) => acc.concat(getTypes(unionType, isType)), []);
  }
...
export const hasNull = (type?: Type): boolean =>
  hasType(type, isNull) || (type !== undefined && getType(type).isNullable());
...
const hasTypeFlag = (type: Type | undefined, flag: TypeFlags): boolean =>
  type !== undefined &&
...
export const hasArray = (type?: Type): boolean => hasType(type, isArray);
export const getArrayTypes = (type?: Type): ReadonlyArray<Type> => getTypes(type, isArray);
...
export const hasTuple = (type?: Type): boolean => hasType(type, isTuple);
export const getTupleTypes = (type?: Type): ReadonlyArray<Type> => getTypes(type, isTuple);
