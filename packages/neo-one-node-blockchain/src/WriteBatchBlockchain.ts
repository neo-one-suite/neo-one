...
export class WriteBatchBlockchain {
  ...
  private async persistUTXOTransactions(
    monitor: Monitor,
    block: Block,
    transactions: readonly (readonly [number, (ContractTransaction | ClaimTransaction | MinerTransaction)])[],
  ...
  private async persistTransactions(
    monitor: Monitor,
    block: Block,
    transactions: readonly (readonly [number, Transaction])[],
    lastGlobalTransactionIndex: BN,
      ...
      .captureSpan(
        async (span) => {
          ...
          if (transaction.type === TransactionType.Register && transaction instanceof RegisterTransaction) {
            ...
            const migratedContractHashes: [UInt160, UInt160][] = [];
            const voteUpdates: [UInt160, readonly ECPoint[]][] = [];
            const actions: (NotificationAction | LogAction)[] = [];
  ...
  private async updateAccount(
    ...
    const balances = values.reduce<{ [asset: string]: BN | undefined }>(
      (acc, [asset, value]) => {
        const key = common.uInt256ToHex(asset);
        const balance = acc[key];
        acc[key] = balance === undefined ? utils.ZERO : balance.add(value);
