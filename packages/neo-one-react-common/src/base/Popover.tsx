...
import { Hidden, Position, HiddenProps, as, Popover as PopoverBase } from 'reakit';
...
class PopoverComponent extends React.Component<PopoverProps, PopoverState> {
  ...
  getOptions = (): Popper.PopperOptions => {
    ...
    const arrow = popover.querySelector(getSelector(PopoverBase.Arrow));
    return {
      placement,
      modifiers: {
        hide: { enabled: false },
        ...
        flip: { enabled: flip, padding: 16, boundariesElement: 'window' },
        preventOverflow: {
          enabled: true,
  ...
  public initPopper = () => {
    if (!this.popper) {
      ...
      if (popover.parentNode) {
        this.popper = new Popper(popover.parentNode as Element, popover, this.getOptions());
      }
    }
  };
  ...
  public componentDidUpdate(prevProps: PopoverProps & Readonly<{ children?: React.ReactNode }>) {
    if (
