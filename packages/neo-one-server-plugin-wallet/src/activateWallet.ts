import { compoundName, InteractiveCLIArgs } from '@neo-one/server-plugin';
import { take } from 'rxjs/operators';
import { constants } from './constants';
import { WalletPlugin } from './WalletPlugin';

export const activateWallet = (plugin: WalletPlugin) => ({ cli }: InteractiveCLIArgs) => {
  const { describe } = plugin.walletResourceType.getCRUD();
  const command = cli.vorpal
    .command(
      'activate wallet <name>',
      'Activates Wallet <name>, setting it as the default wallet for ' + 'other commands.',
    )
    .autocomplete({
