...
import { common, Contract, ContractABI, crypto, privateKeyToAddress, UserAccountID } from '@neo-one/client-common';
...
export const buildTests = async (project: string) => {
  ...
  verifyICOContract(contracts.find((contract) => contract.manifest.name === 'ICO'));
  verifyTokenContract(contracts.find((contract) => contract.manifest.name === 'Token'));
  verifyEscrowContract(contracts.find((contract) => contract.manifest.name === 'Escrow'));
