/* @hash 6ef27ae2aad6ca4477abfcf75bec1f9c */
// tslint:disable
/* eslint-disable */
import {
  addLocalKeysSync,
...
> => {
  ...
  if (process.env.NODE_ENV !== 'production' || process.env.NEO_ONE_DEV === 'true') {
    providers.push({ network: 'local', rpcURL: `http://${host}:10290/rpc` });
  }
...
export const createDeveloperClients = (host = 'localhost'): DeveloperClients => ({
  local: new DeveloperClient(new NEOONEDataProvider({ network: 'local', rpcURL: `http://${host}:10290/rpc` })),
});
