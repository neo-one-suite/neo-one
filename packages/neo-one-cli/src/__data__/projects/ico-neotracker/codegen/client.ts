/* @hash 738771d3d985773a31f97225fbf28aa0 */
// tslint:disable
/* eslint-disable */
import {
  Client,
...
> => {
  ...
  if (process.env.NODE_ENV !== 'production' || process.env.NEO_ONE_DEV === 'true') {
    providers.push({ network: 'local', rpcURL: `http://${host}:10130/rpc` });
  }
...
export const createDeveloperClients = (host = 'localhost'): DeveloperClients => ({
  local: new DeveloperClient(new NEOONEDataProvider({ network: 'local', rpcURL: `http://${host}:10130/rpc` })),
});
