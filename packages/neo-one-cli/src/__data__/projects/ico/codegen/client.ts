/* @hash e9d3d372fc6e63862375991230fb387e */
// tslint:disable
/* eslint-disable */
import {
  Client,
...
> => {
  ...
  if (process.env.NODE_ENV !== 'production' || process.env.NEO_ONE_DEV === 'true') {
    providers.push({ network: 'local', rpcURL: `http://${host}:10150/rpc` });
  }
...
export const createDeveloperClients = (host = 'localhost'): DeveloperClients => ({
  local: new DeveloperClient(new NEOONEDataProvider({ network: 'local', rpcURL: `http://${host}:10150/rpc` })),
});
