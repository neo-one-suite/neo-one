...
export const wireDiagnostics = (manager: MonacoWorkerManager, languageID: string) => {
  ...
  const handleDiagnostics = (diags: readonly { file: string; diagnostics: readonly FlattenedDiagnostic[] }[]) => {
  ...
  const mapDiagnostics = (
    source$: Observable<readonly { file: string; diagnostics: readonly FlattenedDiagnostic[] }[]>,
