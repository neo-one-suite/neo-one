...
export const restore = async ({
  ...
  try {
    ...
    monitor.log({
      ...
      message: 'Skipping restore because it already exists',
    });
  ...
  await monitor.captureSpanLog<Promise<void>>(
    async (span) => {
      await Promise.all<void>([fs.remove(dataPath), fs.remove(tmpPath)]);
      await Promise.all<void>([fs.ensureDir(tmpPath), fs.ensureDir(path.dirname(readyPath)), fs.ensureDir(dataPath)]);
