...
export const NeoNotImplementedError = makeErrorWithCode(
  'NEO_NOT_IMPLEMENTED',
  (name: string) => `${name} not implemented for the NEODataProvider. Use a NEOONEDataProvider to use this method.`,
);
export const UnknownTransactionError = makeErrorWithCode(
  'UNKNOWN_TRANSACTION',
  (transaction: string) => `Unknown transaction: ${transaction}`,
);
