...
}): readonly (ForwardOptions | ForwardValue)[] => {
...
export const filterEvents = (actions: readonly (Event | Log)[]): readonly Event[] =>
  actions.map((action) => (action.type === 'Event' ? action : undefined)).filter(utils.notNull);
export const filterLogs = (actions: readonly (Event | Log)[]): readonly Log[] =>
...
export const getParametersObject = ({
  ...
  const zipped = _.zip(abiParameters, parameters) as [ABIParameter, ContractParameter][];
...
export const convertParams = ({
  ...
  readonly parameters: readonly ABIParameter[];
  readonly params: readonly (Param | undefined)[];
  readonly senderAddress?: AddressString;
}): {
  readonly converted: readonly (ScriptBuilderParam | undefined)[];
  readonly zipped: readonly (readonly [string, Param | undefined])[];
} => {
  const parameters =
