...
The NEO•ONE toolchain generates a helper function `withContracts` in `one/genereated/test.ts` that makes testing a breeze:
...
In addition to the properties listed above, the object will contain a smart contract API object property for each smart contract in your project, configured with the `Client` at the `client` property.

Within your callback to the `withContracts` function, you test your smart contract using the same NEO•ONE client APIs that you use to interact with the contract in production (and that we've discussed over the previous 2 chapters).
