  ...
  entry:
    stage === 'prod'
      ? {
          ...
          'testRunner.worker': path.resolve(EDITOR_PACKAGE, 'src', 'engine', 'test', 'testRunner.worker.ts'),
      ...
      : // async chunks seem broken with webpack dev server in workers
        {
          ...
          'testRunner.worker': path.resolve(EDITOR_PACKAGE, 'src', 'engine', 'test', 'testRunner.init.ts'),
