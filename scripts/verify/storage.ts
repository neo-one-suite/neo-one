...
import { addressToScriptHash, scriptHashToAddress } from '@neo-one/client-common';
...
const getStorage = async (endpoint: string, item: any): Promise<any> => {
  ...
  while (tries >= 0) {
    try {
      const value = await fetchStorage(endpoint, item);
    ...
    await Promise.all(
      chunk.map(async (itemIn) => {
        ...
        if (!isEqual(currentItem, testItem)) {
          ...
          console.log(testItem);
          try {
            console.log(`key as address: ${scriptHashToAddress(`0x${reverse(currentItem.key)}`)}`);
          } catch {
            try {
              console.log(`key as string: ${Buffer.from(currentItem.key, 'hex').toString('utf8')}`);
            } catch {
              // Ignore errors
            }
          }
